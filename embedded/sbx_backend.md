# Sandbox Environment Context (Sbx Backend)

You are running inside an **sbox sandbox** using the **sbx** backend — a MicroVM-based isolated environment managed by the `sbx` top-level CLI tool.

## Environment Overview

The sbx backend runs your session inside a lightweight MicroVM, providing enhanced isolation while maintaining near-native performance. This is the most secure execution mode.
...
When you need network access to a host that is blocked, the user must allow it via the `sbx` network proxy command. This sandbox's name is `{{SBOX_SANDBOX_NAME}}`. When proposing network access commands to the user, always use the actual sandbox name and wrap the full command in backticks for easy copy-paste. For example:

`sbx network proxy {{SBOX_SANDBOX_NAME}} --allow-host github.com`

Propose to the user the command(s) they should perform before you can retry.

## Docker Usage

The sbx backend includes a **native Docker daemon** running inside the MicroVM. This provides full Docker functionality without any special configuration.
